...
var getSmaller = function(triangle, m, n) {
  if (m === 0) return triangle[m][n]

  if (triangle[m - 1][n - 1] === undefined) {
    return getSmaller(triangle, m - 1, n) + triangle[m][n]
  }
  if (triangle[m - 1][n] === triangle[m - 1][n - 1]) {
    const smallerA = getSmaller(triangle, m - 1, n) + triangle[m][n]
    const smallerB = getSmaller(triangle, m - 1, n - 1) + triangle[m][n]
    return smallerA < smallerB ? smallerA : smallerB
  } else if (triangle[m - 1][n] < triangle[m - 1][n - 1]) {
    return getSmaller(triangle, m - 1, n) + triangle[m][n]
  } else (triangle[m - 1][n] > triangle[m - 1][n - 1]) {
    return getSmaller(triangle, m - 1, n - 1) + triangle[m][n]
  }
}
```
