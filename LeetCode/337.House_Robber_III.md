...
function robRecursive(node, cache) {
  ...
  return Math.max(
    node.val
    + (node.left && node.left.left ? rob(node.left.left) : 0)
    + (node.left && node.left.right ? rob(node.left.right) : 0)
    + (node.right && node.right.left ? rob(node.right.left) : 0)
    + (node.right && node.right.right ? rob(node.right.right) : 0),
    (node.left ? rob(node.left) : 0)
    + (node.right ? rob(node.right) : 0)
  )
}
```
