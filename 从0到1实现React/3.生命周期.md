...
getSnapshotBeforeUpdate(prevProps, prevState): 可以将结果传入 componentDidUpdate 里，从而达到数据统一，可以替代 componentWillUpdate()（缺点就是前面讲的异步渲染，componentWillUpdate() 与 componentDidUpdate() 间数据会不统一）
```
